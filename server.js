...
Socket.IO bağlantısı:
  const socket = io('http://localhost:3001', { auth: { username: 'Ceka' } });

Oda oluşturma:
  socket.emit('room:create', { language: 'tr', category: 'meyveler', maxPlayers: 8 }, (res)=>console.log(res));

Odaya katılma:
  socket.emit('room:join', { roomId: 'abcd12' }, (res)=>console.log(res));

Oyunu başlat (sadece host):
  socket.emit('game:start', { roomId: 'abcd12' });

Çizim yayınlama (çizen):
  socket.emit('draw:data', { roomId: 'abcd12', payload: { type:'stroke', points:[...] } });

Chat ve tahmin:
  socket.emit('chat:send', { roomId: 'abcd12', text: 'elma' });

WebRTC sesli sohbet (işaretleşme):
  // aynı odadaki herkese peer-to-peer bağlanmak için offer/answer/ice olaylarını hedefId ile gönderin.
  socket.on('webrtc:offer', ({ fromId, offer }) => { /* RTCPeerConnection.setRemoteDescription */ });
  socket.on('webrtc:answer', ({ fromId, answer }) => { /* RTCPeerConnection.setRemoteDescription */ });
  socket.on('webrtc:ice', ({ fromId, candidate }) => { /* pc.addIceCandidate */ });
  // İstemci tarafında STUN/TURN ayarları girilmeli. Örn:
  // new RTCPeerConnection({ iceServers: [{ urls: 'stun:stun.l.google.com:19302' }] })

UI güncellemeleri için:
  socket.on('room:state', (state) => { /* oyuncular, skor, maske, süre */ });
  socket.on('round:start', (data) => { /* maske, çizen kim, süre sonu */ });
  socket.on('round:start:drawer', (data) => { /* gerçek kelime sadece çizene gelir */ });
  socket.on('round:end', (data) => { /* kelimeyi göster, bir sonraki raunda geçiş */ });
  socket.on('game:over', ({ results }) => { /* sıralama tablosu */ });
  socket.on('guess:correct', ({ by, award, total }) => { /* bildirim ve skor */ });
  socket.on('system', (msg) => { /* odaya giriş/çıkış mesajları */ });

Güvenlik/Üretim önerileri:
  - Gerçek veritabanı ve JWT ile auth ekleyin; Socket.IO middlware ile token doğrulayın.
  - Oda kodu çakışmalarına karşı daha güçlü ID üretin (nanoid v4 gibi).
  - Çizim verisini sınırlandırın (boyut/frekans), hileye karşı çizenin kelimesini istemciye göndermeyin (zaten sadece çizene gidiyor).
  - WebRTC için TURN sunucusu ekleyin (örn. coturn) NAT arkası kullanıcılar için şart olabilir.
*/
